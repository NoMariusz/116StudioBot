...
async def display_michal_lol_time(client, message: discord.Message, **kwargs):
    ...
    await message.channel.send("Michał (critovork) :hot_face: :foot: has play in lol %s hours overall :thinking:\n and has play in lol %s hours from last meeting at %s\n this is %s hours per day :scream:"
...
commands_dict = {
    COMMAND_BOT_SIGN + "stupka": {"description": "Display who like foots", "command": on_stupki, "parametrized": False},
    ...
    COMMAND_BOT_SIGN + "set_next_meeting": {"description": "Set date of next meeting, please use date in format %s\nRemember: set date in UTC time zone" % DATETIME_FORMAT, "command": set_next_meeting_date, "parametrized": True},
    COMMAND_BOT_SIGN + "next_meeting": {"description": "Display when is next meeting\nRemember: date should be in UTC time zone", "command": display_next_meeting_date, "parametrized": False}
}
